 ...
 * committee/appeal flow above — this hits GenLayer Studio (studionet) for
 * real, so it only renders once VITE_MERIDIAN_ADJUDICATOR /
 * VITE_MERIDIAN_OUTBOX are set (see .env.example).
 */
export function GenlayerVerifyPanel({ escrow }: { escrow: Escrow }) {
  ...
  return (
    <div className="space-y-3 rounded-xl border border-dashed border-fg/20 bg-surface p-4 shadow-[var(--shadow-border)]">
      <p className="text-sm text-fg">Verify on live GenLayer Studio</p>
      <p className="text-sm leading-relaxed text-muted">
        Calls the real deployed contract at {adjudicator.slice(0, 10)}… on GenLayer Studio
        (studionet) with this case's facts, and returns the genuine leader/validator consensus
        verdict — separate from the simulated committee above.
      </p>
      <Button
        variant="secondary"
        onClick={run}
